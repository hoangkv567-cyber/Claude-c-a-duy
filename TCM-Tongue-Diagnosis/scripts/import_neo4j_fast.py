...
def main():
    ...
    for _, row in df.iterrows():
        ...
        hoichung_baithuoc_batch.append({'hoichung': hoichung, 'bai_thuoc': bai_thuoc})
        
        if pd.notna(row['triệu_chứng']):
            for symptom in [s.strip() for s in row['triệu_chứng'].split(',') if s.strip()]:
                hoichung_trieuchung_batch.append({'hoichung': hoichung, 'symptom': symptom})
                
        if pd.notna(row['vị_thuốc']):
            for herb in [h.strip() for h in row['vị_thuốc'].split(',') if h.strip()]:
                baithuoc_vithuoc_batch.append({'bai_thuoc': bai_thuoc, 'herb': herb})
    ...
    with driver.session() as session:
        ...
        """, batch=benh_hoichung_batch)

        print("Đang nhập Bài thuốc...")
        session.run("""
            UNWIND $batch AS row
            MATCH (h:HoiChung {name: row.hoichung})
            MERGE (p:BaiThuoc {name: row.bai_thuoc})
            MERGE (h)-[:ĐƯỢC_ĐIỀU_TRỊ_BẰNG]->(p)
        """, batch=hoichung_baithuoc_batch)
        ...
        for i in range(0, len(baithuoc_vithuoc_batch), chunk_size):
            ...
            session.run("""
                UNWIND $batch AS row
                MATCH (p:BaiThuoc {name: row.bai_thuoc})
